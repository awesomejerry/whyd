...
□ 檢查 CSS：
  - 所有顏色是否使用 var(--text), var(--surface), var(--bg)？
  - 有沒有硬編碼的顏色值（#333, black, white）？
```
...
□ 英文模式：
  - 切換到英文
  - 所有文字是否翻譯（包括相對時間如「2 小時前」）
  - 新增文字是否已加入 locales/zh-TW.json 和 locales/en.json
```

### 3. 功能測試
```
□ JS 語法正確：node --check js/*.js
□ 本地伺服器測試無誤
```
